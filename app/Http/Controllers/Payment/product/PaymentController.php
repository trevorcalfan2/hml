...
class PaymentController extends Controller
{
    ...
    {
        return redirect()->route('front.checkout')->with('error', 'Pago cancelado.');
    }

    public function qrPayCancle()
    {
        return redirect()->route('front.multipurpose.qrmenu.checkout')->with('error', 'Pago cancelado.');
    }
    ...
    public function orderValidation($request) {
        ...
        if (!Session::has('cart')) {
            return back()->with('error', 'Ningún artículo añadido al carrito!');
        }
      ...
      // check if any of the start or end time is empty,
        ...
        if (empty($start) || empty($end)) {
            return back()->with('error', "Cerramos los " . $todaysDay)->withInput($request->all());
        }
        ...
        if (str_contains($orderTime->start_time, 'PM') && str_contains($orderTime->end_time, 'AM')) {
            ...
            if ($currentTime < $start || $currentTime > $end) {
                return back()
                    ->with('error', "Tomamos pedidos de " . $orderTime->start_time . " a " . $orderTime->end_time . " los " . $todaysDay)
        ...
        $messages = [
            'billing_fname.required' => 'El campo de nombre de facturación es obligatorio',
            'billing_lname.required' => 'El campo de apellido de facturación es obligatorio',
            'shpping_fname.required' => 'El campo de nombre de envío es obligatorio',
            'shpping_lname.required' => 'El campo de apellido de envío es obligatorio',
            'shpping_address.required' => 'El campo de dirección de envío es obligatorio',
            'shpping_city.required' => 'El campo del país de envío es obligatorio',
            'shpping_country.required' => 'El campo del país de envío es obligatorio',
            'shpping_number.required' => 'El campo del número de teléfono de envío es obligatorio',
            'shpping_email.required' => 'El campo de correo electrónico de envío es obligatorio',
        ];
                ...
                function ($attribute, $value, $fail) use ($request, $be) {
                    if ($be->delivery_date_time_required == 1) {
                        if (!$request->has('delivery_date') || !$request->filled('delivery_date')) {
                            $fail("El campo de fecha de entrega es obligatorio.");
                        ...
                        if ($request->has('delivery_time') && $request->filled('delivery_time')) {
                            ...
                            if (!empty($tf) && $tf->max_orders != 0) {
                                ...
                                if ($orderCount >= $tf->max_orders) {
                                    $fail("El número de pedidos en este período de tiempo ha llegado a su límite");
                                }
                            }
                        }
                    }
                ...
                $rules['receipt'] = [
                    ...
                    function ($attribute, $value, $fail) use ($request) {
                        ...
                        if (!in_array($ext, array('jpg', 'png', 'jpeg'))) {
                            return $fail("Solo se permiten imágenes png, jpg, jpeg");
                        }
                    },
                ];
            }
    ...
    public function mailToAdmin($order) {
        ...
        $body = "Se ha realizado un nuevo pedido.<br>
        <strong>Número de orden: </strong> " . $order->order_number . "<br>
        <a href='" . route('admin.product.details', $order->id) . "'>Haga clic aquí para ver los detalles del pedido</a>";
        $data = [
